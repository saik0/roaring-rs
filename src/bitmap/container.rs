...
impl Container {
    ...
    pub fn ensure_correct_store(&mut self) {
        match (&self.store, self.len) {
            (Store::Bitmap(_), len) if len <= ARRAY_LIMIT => self.store = self.store.to_array(),
            (Store::Array(_), len) if len > ARRAY_LIMIT => self.store = self.store.to_bitmap(),
            _ => (),
        }
    }
}
