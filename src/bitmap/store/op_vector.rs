...
use std::simd::{u16x4, mask16x4, Simd, LaneCount, SupportedLaneCount, SimdElement, u16x8, u16x16, Swizzle2, Which, mask16x8, Mask, u8x16, i8x16, usizex8};
...
mod util {
    use std::mem;
    use std::simd::{LaneCount, mask16x8, Simd, SimdElement, SupportedLaneCount};
    ...
    pub fn lanes_min<U, const LANES: usize>(
        ...
        where U: SimdElement + PartialOrd,
    ...
    #[inline]
    pub fn store<U, const LANES: usize>(
        v: &Simd<U, LANES>,
        out: &mut [U],
    )
        where U: SimdElement + PartialOrd,
    ...
    #[inline]
    pub unsafe fn store_unchecked<U, const LANES: usize>(
        v: &Simd<U, LANES>,
        out: &mut [U],
    )
        where U: SimdElement + PartialOrd,
...
mod shim {
    use std::mem;
    use std::simd::{i8x16, mask16x8, Simd, u16x8, u8x16};
    ...
    pub fn swizzle_bytes(a: &u8x16, b: &u8x16) -> u8x16 {
        ...
        // TODO test
        #[cfg(target_feature = "simd128")] unsafe {
        ...
        let (a, b) = unsafe {
            (mem::transmute::<u8x16, i8x16>(*a).to_array(), mem::transmute::<u8x16, i8x16>(*b).to_array())
        };
        let mut r = [0; 16];
        for i in 0usize..16 {
    ...
    pub fn to_bitmask(mask: &mask16x8) -> usize {
        #[cfg(target_feature = "sse2")] unsafe {
            ...
            return _mm_movemask_epi8(_mm_packs_epi16(mem::transmute(*mask), _mm_setzero_si128())) as usize;
        }
